...
class A():
    ...
    def __init__(self, **params):
        ...
        self.cdag_up = self.mtx_comp(self.mtx_calc, self.up, 0) # self.md - 1
        self.cdag_down = self.mtx_comp(self.mtx_calc_down, self.up, self.f_sites // 2)
        ...
        for state in self.comb_n:
            for inner in state:
                ...
                for i in temp_basis: # Flatten 2d -> 1d
                    fb.append(i)
        return np.array(fb, dtype=np.int32)
    ...
    def mtx_calc(self, n, operator, p):
        '''
        ...
        #  Число состояний 
        R_up = int(fact(self.mc + self.md) / fact(n_up) / fact(self.mc + self.md - n_up))
        R_down = int(fact(self.mc + self.md) / fact(n_down) / fact(self.mc + self.md - n_down))
        R_bose = int(self.n_max + 1)**self.mb   
    ...
    def mtx_calc_down(self, n, operator, p):
        ...
        if self.mc + self.md > n_down:    
            R_down1 = int(fact(self.mc + self.md) / fact(n_down + 1) / fact(self.mc + self.md - n_down - 1))          
...
#                 print(j, p)
    ...
    def _divide_h(self):
        ...
        for block in self.comb_n:
            inner = {}
            for n in block:
                R_up = int(fact(self.mc + self.md) / fact(n[0]) / fact(self.mc + self.md - n[0])) # n[0] - spin up; n[1] - spin down
                R_down = int(fact(self.mc + self.md) / fact(n[1]) / fact(self.mc + self.md - n[1]))
    ...
    def _hamiltonian_not_changed_parts(self):
        ...
        self.a_dag_up = [self.mtx_comp(self.mtx_calc, self.up, i) for i in range(self.md, self.mc + self.md)]
        self.a_up = [i.transpose() for i in self.a_dag_up]
        self.a_dag_down = [self.mtx_comp(self.mtx_calc_down, self.up, i) for i in range(self.mc + 2 * self.md, 2 * (self.mc + self.md))]
